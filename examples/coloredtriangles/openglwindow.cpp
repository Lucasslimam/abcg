...
void OpenGLWindow::paintGL() {
  ...
  if (now >= last_time + (uint64_t)delay) {
    setupModel();
    last_time = now;
  } else if (last_time == 0.00f) {
    setupModel();
    last_time = now;
  }
...
void OpenGLWindow::paintUI() {
...

  {
    ...
    ImGui::Checkbox("Flat colors", &flat_colors);
    ...
    ImGui::SliderFloat("Delay", &delay, 0.0f, 1000.0f);
...
void OpenGLWindow::setupModel() {
  ...
  std::uniform_real_distribution<float> rd2(0.00f, 1.00f);
  auto vetor_flat = glm::vec4{0.36f, 0.83f, 1.00f, 1.0f};

  if (!flat_colors) {
    m_vertexColors[0] = glm::vec4{rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine)};
    m_vertexColors[1] = glm::vec4{rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine)};
    m_vertexColors[2] = glm::vec4{rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine), rd2(m_randomEngine)};
  } else {
    m_vertexColors[0] = vetor_flat;
    m_vertexColors[1] = vetor_flat;
    m_vertexColors[2] = vetor_flat;
  }
